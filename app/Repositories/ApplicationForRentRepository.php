...
class ApplicationForRentRepository
{
    ...
    public function changeRequest($input, $application_for_rent)
    {
        // If request is rejected then update the status and return response
        if($input['status'] == 2)
        {
            $application_for_rent->application_status = $input['status'];
            $application_for_rent->remark = $input['reason'];
            $application_for_rent->save();
            return true;
        }
