...
use App\Http\Requests\Admin\ChangeApplicationForRentStatusRequest;
...
class ApplicationForRentController extends Controller
{
    ...
    public function changeRequest(ChangeApplicationForRentStatusRequest $request, ApplicationForRent $application_for_rent)
    {
        $this->applicationforrentRepository->changeRequest($request->validated(), $application_for_rent);
        return response()->json(['success'=> 'Request '. ($request->status == 1 ? 'approved' : 'rejected') .' successfully!']);
    }
